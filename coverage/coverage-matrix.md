...
| B3 | Transfer of risks | **every rule now carries the surviving `oFailureCosts`** (guarded by `Happens(goodsIdentified)`), with its full per-rule trigger set: nomination failure + vessel/carrier failure (F-terms), B10 notice failure (`oNotifySchedule` violated — all non-F rules), B7-clearance failure (DAP/DPU), take-delivery failure (EXW), and **DDP's B7-assistance failure** (`oAssistSeller` violated — now deadline-bound via L4). **Every premature-transfer limb of the standard is now modelled**; only risk *incidence* stays a language gap | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ |
