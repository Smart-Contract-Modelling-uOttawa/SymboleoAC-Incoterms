...
| O4a | **Conceptualize the identity→role authentication mapping.** In SymboleoAC.ump, `Role` has *no attributes*; the runtime's `authenticate` matches four certificate fields (`HF.name`/`HF.role`/`organization`/`department`) against role data — classic RBAC user-assignment, implemented but not modelled. | The `dept` requirement had to be discovered empirically on-chain ("Unauthorized" with no modelled reason; issue SymboleoAC2SC#1); the CLI enrollment recipe in deploy/README.md §5 encodes knowledge that belongs in the ontology (a `Credential`/assignment concept). **Implemented 2026-07-08** (SymboleoAC-JS-Core#2, second commit): `SymboleoAC.ump` now declares a `Credential` class with `subjectName`/`subjectOrg`/`subjectDept` and an association to `Role`, plus an `Authority` issuer. The `dept` requirement is no longer folklore; the codegen wiring already matches it. | **implemented** (PR pending) |
| O4b | **Surface and exercise the ontology's full AC vocabulary.** `Rule` supports `Grant/Revoke` × `read/write/all/transfer`; the grammar accepts all of them (probe-verified) — yet the corpus, the group's examples, and the docs only ever use `Grant read/write`. `transfer` is exactly the negotiable-document primitive the trade domain needs; `Revoke` gives negative authorization (e.g. cutting a carrier's access after termination). Document them, give them runtime semantics tests, and add codegen/Fabric transactions that *exercise* a transfer. | Wave-3 probes: `Grant transfer To buyer On billOfLading`, `Revoke read`, `Grant all` all compile 0/0; the B/L rules' ACPolicy now records the buyer's transfer right as the endorsability device (A6). **Implemented 2026-07-08** (SymboleoAC-JS-Core#2 + SymboleoAC-IDE PR / SymboleoAC-Web PR "phase3-transfer-txn"): (a) spec-tier constraint rules count at authorization time (previously invisible to `hasPermesstion`); (b) `all` implies read/write/transfer; (c) Revoke is deny-overrides against grants AND structural pre-authorizations; (d) new `ACPolicy.transferResource(res, actor, newOwner)` executes the transfer with permission check + ownership reassignment; (e) codegen now emits an on-chain `transferResource_<var>` Fabric transaction per spec-transferable resource. Owner-conveys-endorsement makes the B/L endorsement chain end to end (sale in transit). **Verified on-chain 2026-07-08**: FOB redeployed to Fabric as v5.0/seq5 on js-core 1.0.27; the buyer exercised its B/L transfer grant on-chain (`transferResource_billOfLading` → `{"successful":true}`, ownership reassigned) as part of a full authorized happy path reaching `SuccessfulTermination` (deploy/README §6). | **implemented + on-chain verified** |
| O4c | **Ontology-internal quality pass.** `Operation.preCondition/postCondition` are typed `Condition` — a type declared nowhere in the ump; association names carry typos (`LegalPositionStateEven`, `ruleAccesseor`, `inputAttributs/outputAttributs`). | Direct reading of SymboleoAC.ump. Cosmetic, but this is the conceptual model of record for a journal-published language. **Implemented 2026-07-08** (SymboleoAC-JS-Core#2, second commit): `Condition` declared with leftSide/op/rightSide (the shape the runtime already uses); four typos fixed. Umple treats association ends as identifiers — downstream regens will pick up renamed accessors. | **implemented** (PR pending) |
...
|----|--------------------|----------|--------|
| L1 | **Conditional/default expressions** (`if-then-else` or a coalesce over terms), esp. for deadlines. | The ICC B10→A2 cascade — "the buyer-selected date governs; otherwise the end of the agreed period" — cannot be written: an `or` of two `WhappensBefore` *loosens* (either bound suffices) instead of *overriding*. We left it unmodelled rather than model it wrongly. Same shape as every "unless otherwise agreed" default. | proposed → **deferred: needs a new grammar rule + parser regeneration; filed SymboleoAC-IDE#6** |
| L2 | **Ordered enumerations / comparable domain values** (e.g. `Enumeration(C < B < A)`). | Insurance cover: amount (110%) and currency are now *checked* constraints, but "at least ICC (C)" is inexpressible — the clause set stays a string. The one item keeping A5 at ◐. **Implemented 2026-07-08 (Phase 2), no code change needed:** enumerations already generate as ordinal objects (`{C:0,B:1,A:2}`) and `PComparison` already emits numeric comparison, so `coverLevel >= ICCClause(C)` works end to end. CIF/CIP `oInsure` now check the cover clause as an ordered enum; **A5 ◐→✅ for CIF/CIP**; witnessed in `phase2-semantics.test.mjs`. Runtime values must be the enum ordinals (as with every enum here). | **verified-works / implemented** |
...
| L4 | **Event-relative deadlines** (see O8) — surface form: `Date.add(eventVar, n, days)` where `eventVar`'s occurrence time is meant. | DDP B7-assistance limb; also the natural form for "reimburse within N days of assistance". **Implemented 2026-07-08 (Phase 2):** no grammar change (arg1 is already `Expression`); the validator accepts a bare Event variable as `Date.add`'s first arg and the generator emits `contract.<var>._timestamp`. Merged (SymboleoAC-IDE#7/Web#11); probe-verified. **Corpus wired 2026-07-08:** the assistance-reimbursement obligations (`oReimburseSellerAssist`/`oReimburseBuyerAssist`, all rules — every rule has ≥1 assistance channel) now carry `WhappensBefore(...Reimbursed, Date.add(...Provided, reimburseDays, days))`; regenerates 0/0, and `phase2-semantics.test.mjs` witnesses both a timely reimbursement (fulfils) and a late one (deadline enforced → unfulfilled). **DDP's B7-assistance B3 limb is now closed (2026-07-09):** the assistance obligations are deadline-bound (`WhappensBefore(...Provided, Date.add(...Requested, assistDays, days))`), and DDP's `b3_triggers` gains `assist_seller_violated` → `oFailureCosts` fires on a buyer B7-assistance failure. **Every premature-transfer limb of the standard is now first-class** (matrix B3). | **implemented + exercised in corpus** |
...
| C8 | **Codegen NPE on a spec with no `ACPolicy` section**: the section is optional in the grammar and the spec validates clean, but `getDefaultControllerACPolicy` dereferences `Model.getAcpolicys()` (null) and generation crashes before emitting anything (so even C4 cannot catch it). A second, sibling NPE lurked in the validator's `checkControllerType` `@Check`. | Found 2026-07-08 probing W13 with minimal specs — the corpus never hits it (all 11 rules declare an ACPolicy). Filed as SymboleoAC2SC#4. **Fixed 2026-07-09** (SymboleoAC-IDE#8 / SymboleoAC-Web#12, bridge redeployed): both sites null-guard a missing ACPolicy as an **empty policy** (`new ACPolicy([])`) rather than requiring the section — the design-correct choice, since AC is an *additive* extension and the grammar makes the section optional. Verified: a no-ACPolicy spec now generates 0-error/empty-policy through the bridge; specs with an ACPolicy are byte-identical. | **fixed** |
...
| T3 | Test network `network.sh` hard-codes `bin-macos`; Fabric 2.2's `fabric-nodeenv` is Node 12 (crashes on modern syntax). Fix/retag upstream. | deploy/README.md fixes 1 and 4. Filed 2026-07-08 as SymboleoAC-HyperledgerFabric-Test-Netwrok#1 and #2. **Fixed 2026-07-09** (Test-Netwrok PR #3): `network.sh` selects `bin/` vs `bin-macos/` by `uname -s`; new idempotent `scripts/fix-nodeenv.sh` retags a modern nodeenv (`:2.5`, Node 22) under `:2.2`; `deployCC` reminds; README documents both. | **fixed** |
...
> **Phase 3 (Phase-2-independent items) status (2026-07-08 evening):
> implemented, PRs pending.** SymboleoAC-JS-Core#2 lands O4b runtime
> semantics (executable transferResource + deny-overrides Revoke +
> all-implication + spec-tier grants recognised at request time), the O4c
> ontology-quality items, and the O4a Credential/Authority concepts.
> SymboleoAC-IDE#5 / SymboleoAC-Web#10 land the codegen half of O4b: an
> on-chain `transferResource_<var>` Fabric transaction is emitted per
> spec-transferable resource (verified selectively on FOB/CFR/CIF — one
> per spec — and absent on EXW/DDP). 11 new js-core unit tests + 2 new
> corpus witnesses exercising CIF's B/L endorsement chain end-to-end on
> the generated policy; the full 151-test suite is unchanged. Merge order:
> JS-Core#2 → IDE#5 → Web#10 → bridge redeploy → re-run Incoterms CI →
> merge here (as with Phase 1). A cheap bug worth flagging for anyone
> touching the codegen next: in the rule AST `rule.getAction()` returns
> the DECISION (grant/revoke) and `rule.getPermission().getName()` returns
> the ontology's Action (read/write/all/transfer).
